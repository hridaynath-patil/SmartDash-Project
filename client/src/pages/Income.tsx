...
import { Plus, X, Trash2, Calendar, IndianRupee } from 'lucide-react';
...
const Income = () => {
  ...
  const handleOpenAdd = () => {
    setTitle('');
    setAmount('');
    // Default to first income category if available
    setCategory(incomeCategories[0]?._id || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addTransaction(title, amt, 'income', category, date);
    handleClose();
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '2rem', fontWeight: 700, color: 'var(--text-main)' }}>Income Management</h2>
          <p className="text-muted" style={{ margin: '0.25rem 0 0 0' }}>Log and track all cash inflows</p>
        </div>
        <button onClick={handleOpenAdd} className="btn btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div className="card" style={{ padding: '1.5rem 2rem', marginBottom: '2rem', display: 'inline-flex', alignItems: 'center', gap: '1.5rem' }}>
        <div style={{ backgroundColor: 'rgba(16, 185, 129, 0.1)', color: '#10b981', padding: '1rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <IndianRupee size={24} />
        </div>
        <div>
          <p className="text-muted" style={{ margin: 0, fontSize: '0.9rem', fontWeight: 500 }}>Total Income Logged</p>
          <h2 style={{ margin: '0.2rem 0 0 0', fontSize: '1.8rem', fontWeight: 700, color: '#10b981' }}>
      ...
      <div className="card" style={{ padding: '2rem' }}>
        <h3 style={{ margin: '0 0 1.5rem 0', fontSize: '1.2rem', fontWeight: 700 }}>Income Records</h3>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-muted)' }}>
                  <th style={{ padding: '1rem 0.5rem', fontWeight: 600 }}>Source/Title</th>
                  ...
                  <tr key={tx._id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '1rem 0.5rem' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', backgroundColor: 'var(--bg-color)', padding: '0.25rem 0.6rem', borderRadius: '12px', fontSize: '0.85rem' }}>
                        ...
                        <span>{tx.category?.name || 'Income'}</span>
                      </span>
                    </td>
                    ...
                    <td style={{ padding: '1rem 0.5rem', fontWeight: 700, color: '#10b981', textAlign: 'right', fontSize: '1rem' }}>
        ...
        <div style={{ 
          ...
          <div className="card" style={{ width: '100%', maxWidth: '450px', padding: '2rem', animation: 'fadeInUp 0.2s ease-out' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.3rem', fontWeight: 700 }}>Add Income Transaction</h3>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                <label style={{ fontWeight: 600, fontSize: '0.85rem' }}>Description/Source</label>
                <input 
                  type="text" 
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                ...
                <button type="submit" className="btn btn-primary" style={{ flexGrow: 1 }} disabled={incomeCategories.length === 0}>Add Income</button>
              </div>
